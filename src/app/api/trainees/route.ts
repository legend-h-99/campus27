...
export async function GET(request: NextRequest) {
  const check = await guardRequest(request, [PERMISSIONS.TRAINEES_VIEW], "trainees", "GET");
...
export async function POST(request: NextRequest) {
  const check = await guardRequest(request, [PERMISSIONS.TRAINEES_CREATE], "trainees", "POST");
  if (!check.ok) return check.response;
